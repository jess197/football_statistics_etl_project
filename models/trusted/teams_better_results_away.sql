...
ranked_teams AS (
  SELECT team_name
       ...
       , SUM(fs.fixtures_wins_away) AS fixt_wins_away_season
       , ROW_NUMBER() OVER (PARTITION BY league_season ORDER BY SUM(fs.fixtures_wins_away) DESC) AS row_num
  FROM curated_fixtures_alltime_statistics fs
  JOIN br_teams bt ON fs.team_id = bt.team_id
  GROUP BY team_name, league_season
)
SELECT team_name, league_season, fixt_played_season, fixt_played_away_season, fixt_wins_away_season
FROM ranked_teams
WHERE row_num <= 3
ORDER BY league_season, fixt_wins_away_season DESC